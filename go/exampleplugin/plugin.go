...
import (
	...
	"github.com/flyteorg/flytepluginexample/gen/pb-go/proto"
)
...
func (m mySamplePlugin) BuildResource(ctx context.Context, taskCtx pluginsCore.TaskExecutionContext) (k8s.Resource, error) {
	...
	exampleJob := proto.ExamplePluginTask{}
	err = utils.UnmarshalStruct(taskTemplate.GetCustom(), &exampleJob)
	if err != nil {
		return nil, errors.Wrapf(err, "invalid task specification for taskType [%s]", exampleTaskType)
	}
